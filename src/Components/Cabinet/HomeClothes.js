...
import { addCart, addCart_button, changeCabinet, changeCategory, increament, proceedCart, proceedCart_button, removeAll } from '../../actions';
                            ...
                            <div className="collection_cloth_div" style={{ marginBottom: "50px" }}>
                                <div className="collection_img__div">
                                    <Slider {...settings} >
                                        {
                                            item.images.map(res => {
                                                return <div><Link to="/БАЗОВАЯ"><img onClick={() => {dispatch(changeCabinet(item)); dispatch(changeCategory(clothList))}} src={res.img} alt="error img" className="collection_img" /></Link></div>
                                            })
                                        }
                                    </Slider>
                                </div>
